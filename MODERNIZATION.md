...
| 63 | `XADZipParser.[hm]` | performance | Parse the zip central directory from a one-shot in-memory copy instead of reading each CD record from the file handle interleaved with the per-entry local-header seek and a seek-back. `readCentralDirectoryRecord` is split into a handle-parameterised variant (additive — the existing `XADZipParserTests` keep calling the file-handle version), the CD span is bounded by the EOCD position (not the encoder-controlled `centralsize`), gated to single-volume with a 256 MB cap, and falls back to the streaming handle on any oversize/short/multi-volume case (fallback verified SHA-identical). Byte-identical parse/extract including lying-centralsize archives. Measured on 2000-entry archives: warm open −13-16%, cold (sudo purge) −16-18% — the win is the removed stdio thrash/seek-back, not fewer bytes (aggressive read-ahead reads ~the whole file cold either way); the round-trip reduction is what a network volume sees. 200-entry archives unchanged. | upstreamable |
